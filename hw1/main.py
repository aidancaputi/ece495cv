...
def main():
    ...
    for hyperparameters in hyperparameters_to_try:
        loss = train2('adder', hyperparameters, 0.01, 1000, 0.8)
        if(loss < best_loss):
            best_loss = loss
    ...
    for lr in learning_rates_to_try:
        loss = train2('adder', best_hyperparameters, lr, 1000, 0.8)
        if(loss < best_loss):
            best_loss = loss
            best_lr = lr
    ...
    print("\nSo, the best network I tested for the adder dataset has the following details:")
